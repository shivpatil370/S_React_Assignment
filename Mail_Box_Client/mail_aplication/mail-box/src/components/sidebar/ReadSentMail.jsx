...
useEffect(() => {
    const fetchContent = async () => {
      try {
        const response = await fetch(
          `https://mail-box-api-default-rtdb.firebaseio.com/${cleanedEmail}/sentbox/${id}.json`);
        const rawData = await response.json();
             setDate(rawData.todayDate)
             setEmail(rawData.to);
             setSubjet(rawData.subject);
             setIsRead(rawData.isNotReadMail);
            //  console.log(rawData.isNotReadMail)
    ...
    useEffect(()=>{
         if(isRead){
           fetch(`https://mail-box-api-default-rtdb.firebaseio.com/${cleanedEmail}/sentbox/${id}.json`,{
            method:'PATCH',
            body:JSON.stringify({
              isNotReadMail:false
            })
  ...
  const handleDelete=async()=>{
    ...
    if(id&&cleanedEmail){ 
   await fetch(`https://mail-box-api-default-rtdb.firebaseio.com/${cleanedEmail}/sentbox/${id}.json`,{
      method:'DELETE'
     })
     .then((res)=>{
      return res.json();
  ...
  return (
    <div>
      ...
      <div className='d-flex gap-4'>
        ...
        <div onClick={()=>navigate("/sentmail")} title='back to sent mail'>
